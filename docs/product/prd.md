...
Next.js App Router에서는 `npx anlyx dev`가 개발 전용 Next server bridge와 capture helper를 준비하는 것을 기본 경로로 한다. 앱 코드 수정이 어려운 특수 환경이나 디버깅에서는 다음 raw script tag를 fallback으로 사용할 수 있다.

```html
<script src="http://localhost:4777/_anlyx/capture.js" defer></script>
```

기존 `AnlyxDevOverlay` helper는 호환성 경로로만 남길 수 있으며, production에서는 아무것도 렌더링하지 않아야 하고 기본 분석 화면이 되어서는 안 된다.
