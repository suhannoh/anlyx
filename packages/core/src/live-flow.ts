...
export type BrowserRequestEvent = {
  ...
  contextId?: string;
  priority?: FlowRecordPriority;
  runtimeSource?: FlowRecordRuntimeSource;
};
...
export type BackendObservedSpan = {
  id: string;
  parentId?: string;
  type: Exclude<FlowLayerType, "action" | "result">;
  label: string;
  nodeId?: string;
  filePath?: string;
  lineNumber?: number;
...
export type FlowRecordPriority = "primary" | "background";
export type FlowRecordRuntimeSource = "browser" | "server" | "unknown";
...
export function buildFlowRecordFromBrowserEvent(
  ...
  const record: FlowRecord = {
    ...
    priority: event.priority ?? (event.action ? "primary" : "primary"),
    runtimeSource: event.runtimeSource ?? "browser",
...
function buildBlockedDecisionLayer(eventId: string, status: number): FlowLayer {
  if (status === 409) {
    return {
      ...
      evidence: [
        "inferred: 409 response suggests a business decision stop along the source-matched path",
        "browser_observed: response status was captured by the browser",
        "No server runtime trace is available for this request"
      ],
      status
    };
  }
  ...
  return {
    ...
    evidenceLevel: "inferred",
    evidence: [
      `inferred: ${status} response suggests an auth or policy stop along the source-matched path`,
      "browser_observed: response status was captured by the browser",
      "No server runtime trace is available for this request"
    ],
    status
...
function evidenceTextForScannedNode(options: {
  ...
  return [`${level}: layer came from imported project source, not server runtime tracing`];
}
...
function collectRecordEvidence(match: EndpointMatch): string[] {
  ...
  if (match.state === "ambiguous") {
    return [
      "browser_observed: request was captured from the local browser",
      "not_proven: multiple imported endpoints matched the request shape"
    ];
  }

  return [
    "browser_observed: request was captured from the local browser",
    "not_proven: no imported endpoint matched the request"
  ];
}
