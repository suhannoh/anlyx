...
function createViewerFallbackConfig(): NormalizedAnlyxConfig {
  return {
...
type LoadedProjectData = { data: ProjectData; path: string };

async function ensureProjectData(options: {
  projectDataPath: string;
  splitProjectDataPath: string;
  dependencies: RequiredDevCommandDependencies;
...
function isMissingProjectDataError(error: unknown): boolean {
...
export async function readProjectData(path: string): Promise<ProjectData> {
  let content: string;
  ...
  try {
    ...
    throw new Error(
      `Failed to parse Anlyx project data JSON at ${path}: ${
  ...
  try {
    ...
    throw new Error(
      `Invalid Anlyx project data at ${path}: ${
...
async function readSplitProjectData(indexPath: string, index: unknown): Promise<ProjectData> {
...
async function readOptionalProjectJson(path: string): Promise<unknown | undefined> {
  let content: string;
  ...
  try {
    ...
    throw new Error(
      `Failed to parse split Anlyx project JSON at ${path}: ${
        error instanceof Error ? error.message : "invalid JSON"
      }`
    );
  }
...
export async function readReportData(path: string): Promise<ScanResult> {
  ...
  try {
    ...
    if (isNodeError(error) && error.code === "ENOENT") {
      throw new Error(
        `Anlyx report data not found: ${path}. Create or import "anlyx.project.json" first.`
      );
    }
        ...
        if (request.method === "GET" && requestUrl === "/_anlyx/events/stream") {
          if (!liveEvents) {
            sendJsonError(
              response,
              404,
              new Error("Legacy live events are not available for project data.")
            );
            return;
          }
...
export function createLiveEventRuntime(reportData: ScanResult) {
  const streams = new Set<ServerResponse>();
  const recordsByRequestId = new Map<string, FlowRecord>();
  const pendingSpansByRequestId = new Map<string, BackendObservedSpan[]>();
  let latestPageContext: PageContextEvent | undefined;
  let recentRecords: FlowRecord[] = [];

  const rememberRecord = (record: FlowRecord) => {
  ...
  const broadcastRecord = (record: FlowRecord) => {
  ...
  const broadcastPageContext = (event: PageContextEvent) => {
  ...
  return {
    openStream(request: IncomingMessage, response: ServerResponse) {
      response.statusCode = 200;
      setCorsHeaders(response);
    ...
    async recordBrowserRequest(request: IncomingMessage, response: ServerResponse) {
      let payload: unknown;
    ...
    async recordPageContext(request: IncomingMessage, response: ServerResponse) {
      let payload: unknown;
    ...
    async recordBackendSpans(request: IncomingMessage, response: ServerResponse) {
      let payload: unknown;
...
function sendAccepted(response: ServerResponse, value: unknown) {
...
function sendJsonError(response: ServerResponse, statusCode: number, error: unknown) {
...
async function readJsonRequestBody(request: IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    throw new Error(
      `Failed to parse Anlyx runtime event JSON: ${
        error instanceof Error ? error.message : "invalid JSON"
      }`
    );
  }
...
function writeFlowSseEvent(response: ServerResponse, record: FlowRecord) {
  response.write("event: flow\n");
  response.write(`data: ${JSON.stringify(record)}\n\n`);
}

function writePageContextSseEvent(response: ServerResponse, event: PageContextEvent) {
...
function withRequestPageContext(
  event: BrowserRequestEvent,
  request: IncomingMessage,
  latestPageContext?: PageContextEvent
): BrowserRequestEvent {
  const pageUrl =
    event.pageUrl ?? pageUrlFromReferer(request.headers.referer) ?? latestPageContext?.pageUrl;
  if (!pageUrl) {
    return event;
  }
...
function pageUrlFromReferer(referer: string | string[] | undefined): string | undefined {
...
function pageContextId(pageUrl: string): string {
  try {
...
function isPageContextEvent(value: unknown): value is PageContextEvent {
  if (!value || typeof value !== "object") {
    return false;
  }

  const event = value as Partial<PageContextEvent>;
  return (
    event.type === "page_context" &&
