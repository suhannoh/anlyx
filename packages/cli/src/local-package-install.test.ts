...
describe("local package install helper", () => {
  it("prints one generic install command for a version-aligned Anlyx package set", async () => {
    await withTempDir(async (dir) => {
      ...
      expect(stdout).toContain(join(dir, "anlyx-core-0.1.5.tgz"));
      expect(stdout).toContain(join(dir, "anlyx-ui-0.1.5.tgz"));
      expect(stdout).toContain(join(dir, "anlyx-0.1.5.tgz"));
