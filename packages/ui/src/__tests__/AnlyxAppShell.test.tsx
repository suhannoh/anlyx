...
describe("AnlyxAppShell", () => {
  ...
  it("renders Flow JSON fields in the inspector", () => {
    render(<AnlyxAppShell data={flowJsonInspectorResult()} />);

    const inspector = screen.getByRole("complementary", { name: "Inspector" });

    expect(within(inspector).getByText("agent-inferred")).toBeTruthy();
    expect(within(inspector).getByText("estimate")).toBeTruthy();
    expect(within(inspector).getByText("agent inferred from source")).toBeTruthy();
    expect(within(inspector).getByText(/keyword/)).toBeTruthy();
    expect(within(inspector).getByText(/statusCodes/)).toBeTruthy();
...
function flowJsonInspectorResult(): ScanResult {
  const [firstFlow, ...remainingFlows] = mockScanResult.flows;
  const flowNodes = firstFlow!.nodes;
  const defaultNodeId = firstFlow!.mainPath[1] ?? firstFlow!.mainPath[0];
        ...
        nodes: flowNodes.map((node) =>
          node.id === defaultNodeId
            ? {
                ...
                timing: {
                  kind: "estimate",
                  reason: "agent inferred from source"
                },
                request: {
                  query: {
                    keyword: "string"
